...
const Footer = () => {
    ...
    return <Box height={screenSizeDownSm ? "700px" : "500px"} position={"relative"}>
        <Box position={"absolute"} height={screenSizeDownSm ? "400px" : "300px"} width={"80vw"} bottom={"30%"} left={"10vw"} bgcolor={"#f3f7ff"}>
            <Box position={"relative"} width={"100%"} height={"100%"} >

                <img src={images.SideImg} style={{ position: "absolute", width: "150px", opacity: 0.7 }} />
                <Grid container alignItems={"center"} height={"100%"} columnSpacing={10} justifyContent={"space-around"} >
                    <Grid item xs={10} md={7} zIndex={10}>
        ...
        <Box position={"relative"} zIndex={-1} height={"50%"} bottom={0} sx={{ backgroundImage: `url(${images.BackgroundImage})` }}>
            <Grid container position={"absolute"} bottom={0} my={3} alignItems={"center"} justifyContent={"space-between"}>
                ...
                <Grid item container xs={12} md={5} columnGap={3} justifyContent={"center"} mt={2} sx={{
                    ...
                    <Typography onClick={() => console.log("home")}>Home</Typography>
